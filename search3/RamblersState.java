...
public class RamblersState extends SearchState{
    ...
    ArrayList<SearchState> getSuccessors(Search searcher) {
        ...
        int localcost = 0;
        // up
        if (coords.getx() > 0 && !isDuplicated(searcher, coords.getx() - 1, coords.gety())) {
        ...
        // down
        if (coords.getx() < terrainMap.getWidth() - 1 && !isDuplicated(searcher, coords.getx() + 1, coords.gety())) {
            successors.add(new RamblersState(terrainMap, new Coords(coords.gety(), coords.getx() + 1), calculateLocalCost(coords.getx() + 1, coords.gety())));
        }

        // left
        if (coords.gety() > 0 && !isDuplicated(searcher, coords.getx(), coords.gety() - 1)) {
        ...
        // right
        if (coords.gety() < terrainMap.getDepth() - 1 && !isDuplicated(searcher, coords.getx(), coords.gety() + 1)) {
            successors.add(new RamblersState(terrainMap, new Coords(coords.gety() + 1, coords.getx()), calculateLocalCost(coords.getx(), coords.gety() + 1)));
        }
        return successors;
    }
